...
async function runAuthTests() {
  ...
  await db.execute(
    `INSERT INTO bank_connections (id, user_id, bank_name, aspsp_name, aspsp_country, session_id_enc, valid_until, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [conn1Id, user1Id, "Santander", "Santander", "ES", "enc_session", "2099-01-01T00:00:00.000Z", now]
  );
  await db.execute(
    `INSERT INTO accounts (id, connection_id, iban, alias, currency, last_balance, synced_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [acc1Id, conn1Id, "ES1234567890", "Alice Account", "EUR", JSON.stringify([{ amount: "5000.00", currency: "EUR" }]), now]
  );
