...
async function runTest(): Promise<void> {
  ...
  await db.delete(transactions).where(like(transactions.accountId, "dash-acc-%"));
  await db.delete(accounts).where(like(accounts.id, "dash-acc-%"));
  await db.delete(bankConnections).where(inArray(bankConnections.id, [conn1Id, conn2Id, connExpiredId]));
  ...
  await db.insert(bankConnections).values([
    {
      ...
      aspspName: "Santander",
      aspspCountry: "ES",
      sessionIdEnc: encrypt("mock-sess-1"),
    ...
    },
    {
      ...
      aspspName: "Revolut",
      aspspCountry: "GB",
      sessionIdEnc: encrypt("mock-sess-2"),
      validUntil: futureDate,
      status: "active",
      createdAt: now
    },
    {
      ...
      aspspName: "OldBank",
      aspspCountry: "ES",
      sessionIdEnc: encrypt("mock-sess-3"),
      validUntil: futureDate,
      status: "expired",
      createdAt: now
    }
  ...
  await db.insert(accounts).values([
    {
      ...
      alias: "Cuenta Nómina",
      currency: "EUR",
      lastBalance: JSON.stringify([{ amount: "2500.50", currency: "EUR", type: "CLAV" }]),
      syncedAt: now
    },
    {
      id: "dash-acc-2",
      connectionId: conn2Id,
      iban: "GB29REVO00998812345678",
      alias: "Revolut Main",
      currency: "GBP",
      lastBalance: JSON.stringify([{ amount: "1250.75", currency: "GBP", type: "CLAV" }]),
      syncedAt: now
    },
    {
      id: "dash-acc-3",
      connectionId: connExpiredId,
      iban: "ES0000000000000000000000",
      alias: "Expired Account",
      currency: "EUR",
      lastBalance: JSON.stringify([{ amount: "500.00", currency: "EUR", type: "CLAV" }]),
      syncedAt: now
    }
  ]);
