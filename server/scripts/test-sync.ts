...
async function runVerification(): Promise<void> {
  ...
  await db.insert(bankConnections).values([
    {
      ...
      aspspName: "Santander",
      aspspCountry: "ES",
      sessionIdEnc: encrypt("mock-active-session-id"),
    ...
    {
      id: conn2Id,
      bankName: "Banco Pasado",
      aspspName: "Pasado",
      aspspCountry: "ES",
      sessionIdEnc: encrypt("mock-past-session-id"),
      validUntil: pastDate,
      status: "active",
      createdAt: now
    }
  ...
  const [expiredConn] = await db
    ...
    .where(eq(bankConnections.id, conn2Id))
    .limit(1);
  ...
  const [countBeforeRes] = await db.select({ total: count() }).from(transactions);
  const countBefore = countBeforeRes?.total ?? 0;

  const result2 = await syncService.syncAll();
  console.log("Sync result 2:", result2);

  const [countAfterRes] = await db.select({ total: count() }).from(transactions);
  const countAfter = countAfterRes?.total ?? 0;
  ...
  const accRows = await db
    ...
    .from(accounts)
    .where(inArray(accounts.id, ["mock-acc-1", "mock-acc-2"]));

  for (const acc of accRows) {
    if (!acc.lastBalance || !acc.syncedAt) {
      throw new Error(`Account ${acc.id} has missing last_balance or synced_at`);
    }
    const parsedBalance = JSON.parse(acc.lastBalance);
    if (!Array.isArray(parsedBalance) || parsedBalance[0].amount !== "1500.50") {
      throw new Error(`Account ${acc.id} balance data unexpected: ${acc.lastBalance}`);
    }
  }
  ...
  await db.insert(bankConnections).values({
    id: conn3Id,
    bankName: "Banco Error",
    aspspName: "ErrorBank",
    aspspCountry: "ES",
    sessionIdEnc: encrypt("mock-error-session-id"),
    validUntil: futureDate,
    status: "active",
    createdAt: now
  });
  ...
  const [errorConn] = await db
    .select({ status: bankConnections.status })
    .from(bankConnections)
    .where(eq(bankConnections.id, conn3Id))
    .limit(1);
  ...
  await db.delete(transactions).where(like(transactions.accountId, "mock-acc-%"));
  await db.delete(accounts).where(like(accounts.id, "mock-acc-%"));
  await db.delete(bankConnections).where(inArray(bankConnections.id, [conn1Id, conn2Id, conn3Id]));
